...
############################################################
##print(*map(lambda x: x*x, map(round, map(float,input().split()))))
...
##row=[sum(d[x])for x in range(len(d))]
##d_col=[[d[y][x]for y in range(N)]for x in range(N)]
##col=[sum(d_col[x]) for x in range(len(d_col))]
